...
function readOrder(){
    ...
    order.on("child_added",function(data){
        ...
        document.getElementById("cardSection").innerHTML+=`
            <div class="card mb-3">
                <div class="card-body>
                    <h4 class="card-title">Order: ${orderValue.order}</h4>
                    <p class="card-text">Total: ${orderValue.total}</p>
                    <button type="submit" style="color:white;" class="btn 
                    btn-warning" onclick="updateOrder(${orderValue.id},'${orderValue.order}',
                    '${orderValue.total}')"><i 
                    class="fas fa-edit"></i> Edit Order</button>
                    <button type="submit" class="btn 
                    btn-danger" onclick="deleteOrder(${orderValue.id})"><i 
                    class="fas fa-trash-alt"></i> Delete Order</button>
                </div>
            </div>
        `;
    });
...
function reset(){
    ...
    <form class="border p-4 mb-4" id="form">
        ...
        class="btn btn-primary"><i class="fas fa-plus"></i> Add Order </button>
        <button style="display: none;" id="button2"
        class="btn btn-success">Update Order</button>
...
function updateOrder(id,order,total){
    ...
    <form class="border p-4 mb-4" id="form2">
        ...
        <button style="display: none;" type="submit" id="button1" 
        class="btn btn-primary"><i class="fas fa-plus"></i> Add Order </button>
        <button id="button2"
        class="btn btn-success">Update Order</button>
        <button id="button3"
